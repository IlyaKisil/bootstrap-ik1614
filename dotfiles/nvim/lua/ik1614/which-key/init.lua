...
local n_leader = {
  ...
  r = {
    ...
    f = {':lua require("ik1614.fzf-lua").lsp_code_actions()<CR>', "Refactor"},
    r = {':w<CR>:luafile %<CR>', "Run currentl lua file"},
    l = {':call functions#exec_current_line()<CR>', "Run current lua line"},
  },
  s = {
    name = "Show",
    a = {':lua require("ik1614.fzf-lua").lsp_code_actions()<CR>', "Code actions"},
    s = {':lua require("ik1614.fzf-lua").lsp_document_symbols()<CR>', "Symbols"},
    S = {':lua require("ik1614.fzf-lua").lsp_live_workspace_symbols()<CR>', "Workspace symbols"},
    -- d = {':Telescope diagnostics bufnr=0<CR>', ""},
    d = {':lua require("ik1614.fzf-lua").lsp_document_diagnostics()<CR>', "Diagnostics"},
    -- D = {':Telescope diagnostics line_width=120<CR>', ""},
    D = {':lua require("ik1614.fzf-lua").lsp_workspace_diagnostics()<CR>', "Workspace diagnostics"},
    l = {':lua vim.diagnostic.open_float()<CR>', "Line diagnostics"},
    n = {'<cmd>lua vim.diagnostic.goto_next()<CR>', "Next diagnostic"},
    e = {'<cmd>lua vim.diagnostic.goto_prev()<CR>', "Previous diagnostic"},
    i = {':lua require("ik1614.fzf-lua").lsp_implementations()<CR>', "Implementations"},
    -- The following could go just under 'g'
    o = {':lua require("ik1614.fzf-lua").lsp_definitions()<CR>', "Definitions"},
    O = {':lua require("ik1614.fzf-lua").lsp_typedefs()<CR>', "Type definitions"},
    r = {':lua require("ik1614.fzf-lua").lsp_references()<CR>', "References"},
  },
  f = {
    name = "Find (fzf)",
    f = {':<C-u>FzfLua files<CR>', "Files"},
    b = {':<C-u>FzfLua buffers<CR>', "Buffers"},
    s = {':lua require("ik1614.fzf-lua").grep_no_ignore()<CR>', "Search all"},
